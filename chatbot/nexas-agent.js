(function () {
  // ============================================================
  //  NEXAS AI AGENT — Widget Standalone v1.0
  //  Configureaza API_KEY si WHATSAPP_NUMBER mai jos
  // ============================================================
  ...
  const SYSTEM_PROMPT = `Ești ${CONFIG.AGENT_NAME}, asistentul virtual al agenției ${CONFIG.AGENCY_NAME} — o agenție digitală din România specializată în crearea de site-uri web profesionale, optimizare SEO și managementul social media.

PERSONALITATE:
- Vorbești în română, profesionist dar prietenos
- Ești concis și la obiect — max 3-4 propoziții per răspuns
- Ești entuziast față de rezultate concrete pentru business-uri românești

SERVICIILE NEXAS:
1. CREARE SITE WEB — site-uri de prezentare, magazine online (WooCommerce), landing page-uri. Prețuri de la 500€. Timp de livrare: 7–21 zile.
2. SEO (Optimizare Google) — audit SEO, optimizare on-page, link building, rapoarte lunare. Abonamente de la 200€/lună.
3. SOCIAL MEDIA MANAGEMENT — strategie, creare conținut, programare postări, rapoarte. Abonamente de la 150€/lună.
4. AGENT AI CUSTOMER SUPPORT — chatbot AI personalizat pentru site-ul clientului, răspunde 24/7. De la 200€/lună.

REGULI:
- Dacă cineva întreabă prețuri exacte sau vrea o ofertă → dai prețurile de pornire și îl trimiți la WhatsApp
- Nu promite lucruri pe care agenția nu le oferă
- Dacă nu știi răspunsul → spune sincer și trimite la WhatsApp
- La final de conversație relevantă → sugerează contactul pe WhatsApp: +${CONFIG.WHATSAPP}`;

  // ── CSS ──────────────────────────────────────────────────────
  const CSS = `
    @import url('https://fonts.googleapis.com/css2?family=Syne:wght@600;700&family=DM+Sans:wght@300;400;500&display=swap');

    #nexas-widget * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'DM Sans', sans-serif; }
    ...
    #nx-btn:hover { transform: scale(1.08); box-shadow: 0 12px 40px rgba(124,58,237,.65); }
    ...
    #nx-panel {
      ...
      box-shadow: 0 24px 80px rgba(0,0,0,.7), 0 0 0 1px rgba(139,92,246,.08);
      margin-bottom: 12px;
      animation: nxSlideIn .25s ease;
      overflow: hidden;
    }
    ...
    /* HEADER */
    .nx-header {
      background: linear-gradient(135deg,#1a1025,#150f2a);
      padding: 16px 18px;
    ...
    .nx-hinfo { flex: 1; }
    .nx-hname { font-family:'Syne',sans-serif; font-size:14px; font-weight:700; color:#f5f3ff; }
    .nx-hstatus {
    ...
    .nx-close {
      ...
      width:30px; height:30px; cursor:pointer; color:#9ca3af;
      display:flex; align-items:center; justify-content:center;
      font-size:18px; transition:background .15s, color .15s;
    }
    .nx-close:hover { background:rgba(255,255,255,.12); color:white; }
    ...
    .nx-msg-a .nx-bubble {
      ...
      max-width: 88%; display: inline-block; white-space: pre-wrap;
    }
    ...
    .nx-msg-u .nx-bubble {
      ...
      padding: 10px 14px; font-size: 13.5px; line-height: 1.55;
      max-width: 88%; display: inline-block;
    }

    /* TYPING */
    .nx-typing { display:flex; align-items:center; gap:5px; padding:10px 14px; }
    .nx-typing span {
    ...
    .nx-wa {
      ...
      cursor:pointer; color:#10b981; font-size:12.5px; font-weight:500;
    ...
    /* INPUT AREA */
    .nx-input-area {
      padding:12px 14px; border-top:1px solid rgba(139,92,246,.12);
      display:flex; gap:8px; align-items:flex-end;
      background:#0e0e18; flex-shrink:0;
    }
    .nx-input {
      flex:1; background:#1a1a2e; border:1px solid rgba(139,92,246,.2);
      border-radius:12px; padding:10px 14px; color:#f5f3ff;
      font-family:'DM Sans',sans-serif; font-size:13.5px;
      resize:none; outline:none; transition:border-color .15s;
      max-height:90px; min-height:42px; line-height:1.5;
    }
    ...
    .nx-send:hover:not(:disabled){opacity:.88;transform:scale(.97)}
    .nx-send:disabled{opacity:.35;cursor:not-allowed}

    /* FOOTER */
    .nx-footer {
      text-align:center; font-size:10px; color:#374151;
      padding:5px 0 9px; letter-spacing:.4px;
      background:#0e0e18; flex-shrink:0;
    }
    .nx-footer b { color:#6d28d9; font-weight:500; }
  `;
    ...
    root.innerHTML = `
      <div id="nx-panel" class="nx-hidden">
        <div class="nx-header">
          ...
          <div class="nx-hinfo">
            <div class="nx-hname">${CONFIG.AGENT_NAME} — ${CONFIG.AGENCY_NAME}</div>
        ...
        <div class="nx-footer">Powered by <b>Nexas AI</b> · Claude API</div>
      </div>

      <button id="nx-btn" class="nx-pulse" title="Vorbeste cu ${CONFIG.AGENT_NAME}">
  ...
  function initLogic() {
    ...
    const WA_KEYWORDS = ["whatsapp", "+40", "contactati", "scrieti", "suna"];

    function hasWA(text) {
      return WA_KEYWORDS.some(k => text.toLowerCase().includes(k));
    }

    function waUrl() {
      return `https://wa.me/${CONFIG.WHATSAPP}?text=Salut%2C%20am%20o%20intrebare%20despre%20serviciile%20Nexas`;
    }
    ...
    async function sendMessage() {
      ...
      try {
        ...
        const reply = data.content?.[0]?.text || "Îmi pare rău, a apărut o eroare. Te rog contactează-ne pe WhatsApp.";
        history.push({ role: "assistant", content: reply });
        removeTyping();
        addMessage("assistant", reply);
      } catch {
        removeTyping();
        addMessage("assistant", "A apărut o problemă. Te rog scrie-ne pe WhatsApp 👇");
      }
    ...
    function togglePanel() {
      ...
      if (open && history.length === 0) {
        addMessage("assistant", `Salut! Sunt ${CONFIG.AGENT_NAME} de la ${CONFIG.AGENCY_NAME} 👋 Te pot ajuta cu informații despre site-uri web, SEO sau social media. Ce te interesează?`);
